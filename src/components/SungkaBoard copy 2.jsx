...
const SungkaBoard = ({ mode }) => {
  ...
  const handlePitClick = (index) => {
    ...
    const distributeStones = () => {
      if (stones > 0) {
        ...
        checkEndGame(); // Check if the game has ended after each move
      }
    };
  ...
  const renderPit = (stones, label, index, isUlo) => {
    const holesCount = isUlo ? 77 : 35;
    const gridCols = isUlo ? 7 : 7;
    const gridRows = isUlo ? 11 : 5;

    return (
      <div
        key={index}
        onClick={() => handlePitClick(index)}
        className={`flex flex-col items-center justify-center h-${isUlo ? 60 : 40} w-${
          isUlo ? 60 : 40

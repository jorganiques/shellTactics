...
const initialState = {
  ...
  winMessage: '', // New state for winner message
};
...
const gameReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'SET_WIN_MESSAGE':
      return { ...state, winMessage: action.payload }; // Add case for setting win message
      default:
      return state;
  }
};
...
const SungkaBoard = ({ mode }) => {
  ...
  const { board, currentTurn, animating, captureMessage, winMessage } = state;
  ...
  useEffect(() => {
    if (isOutOfMoves(currentTurn)) checkEndGame();
  }, [currentTurn, board]);
  ...
  const handlePitClick = (index) => {
    ...
    const distributeStones = () => {
      if (stones > 0) {
        index = (index + 1) % 16;
        if (shouldSkipUlo(index)) return distributeStones(); // Skip opponent's ulo
        ...
        if (!checkExtraTurn(index)) {
          if (isOutOfMoves(currentTurn)) checkEndGame();
          else switchTurn(); // Switch turns after a move unless the player gets an extra turn
        }
      }
    };
  ...
  const checkCapture = (lastIndex) => {
    ...
    if (isOwnPit && board[lastIndex] === 1 && board[oppositeIndex] > 0) {
      ...
      // Debugging logs
      console.log(`Last Index: ${lastIndex}, Opposite Index: ${oppositeIndex}`);
      ...
      console.log(`Updated Board after capture: ${newBoard}`);
      ...
      dispatch({
        ...
        payload: `${currentTurn} captured pit number ${pitNumber} of the ${opponent}.`,
      });
  ...
  const checkEndGame = () => {
    const [playerAPits, playerBPits] = [board.slice(0, 7), board.slice(8, 15)].map(pits => pits.reduce((acc, stones) => acc + stones, 0));

...
import SungkaShell from '../assets/SungkaShell.png'; // Import the shell image
...
const SungkaBoard = ({ mode }) => {
  ...
  const checkCapture = (lastIndex) => {
    ...
    if (isOwnPit && board[lastIndex] === 1 && board[oppositeIndex] > 0) {
      ...
      if (mode === 'one-player' && currentTurn === 'Player A') {
        console.log('Entering setTimeout block');
        setTimeout(() => {
          console.log('Timeout finished');
          dispatch({ type: 'SET_CAPTURE_MESSAGE', payload: '' });
          switchTurn();
        }, 5000); // 5-second delay before switching turns
      } else {
        console.log('Switching turn immediately');
        switchTurn();
      }
    }
  };
  ...
  const renderPit = (stones, label, index, isUlo) => {
    const holesCount = isUlo ? 98 : 35;
    const gridCols = isUlo ? 7 : 7;
    const gridRows = isUlo ? 14 : 5;

    // Determine the shell color based on animation state
    const shellColor = animating ? '#FF5733' : '#003366'; // Change to a brighter color like orange during animation
    ...
    return (
      <div
        ...
        className={`flex flex-col items-center justify-center h-${isUlo ? 55 : 40} w-${
          isUlo ? 55 : 40
        } text-white font-bold cursor-pointer rounded-full ${isUlo ? 'bg-darkwood' : 'bg-wood'}`}
      >
